...
public class Tower extends GameObject {
    ...
    public Tower(Coordinate towerCoordinate, double width, double height) {
        ...
        for(int i = 0; i != 50; i++){
            ...
            attacker.add(new Attacker(new Coordinate(1000, GameWindow.height / 2 - 40), 100,100, "Circle"));
        }
    }
    ...
    private void paintTower(Graphics2D g2d) {
        ...
        if (attacker.size() > 0) {
            attacker.get(0).paintMe(g2d);
        }
        for(int i = 0; i < attacker.size()-1; i++){
            attacker.get(i+1).paintMe(g2d);
        }
    }
    ...
    public void createAttacker(String form, double xPosition, double yPosition){
        if(form.equals("Circle")){
            attacker.add(new Attacker(new Coordinate(xPosition, yPosition), 100,100, "Circle"));
        }
    }

    public void attackerTouchesCanonball(){
        for(int i = 0; i < missile.size(); i++) {
